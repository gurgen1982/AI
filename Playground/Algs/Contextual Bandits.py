...
class contextual_bandit():
    def __init__(self):
        self.state = 0
        self.bandits = np.array([[0.2, 0, -0.0, -5, 1], [0.1, -5, 1, 0.25, 1], [0, 5, -10, 1, 2], [-5, 5, 5, 5, 1]])
...
# # Define a placeholder for input indices
# indices = tf.compat.v1.placeholder(shape=[None], dtype=tf.int32)

# # Convert indices to one-hot encoding
# one_hot_encoded = slim.one_hot_encoding(indices, num_classes=5)

# with tf.compat.v1.Session() as sess:
#     input_indices = [2, 0, 4, 1]
#     encoded_values = sess.run(one_hot_encoded, feed_dict={indices: input_indices})
