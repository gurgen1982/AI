...
config = {
    "alpha_vantage": {
        ...
        "key_close":'4. close',#gurgen added
    },
    "data": {
        "window_size": 20,
...
def download_data(config):
    ...
    data, meta_data = ts.get_daily(symbol=config["alpha_vantage"]["symbol"], outputsize=config["alpha_vantage"]["outputsize"]) # to use get_daily_adjusted switch to premium. there is other method also. check them out for example get_intraday
...
plt.grid(visible=True, which='major', axis='y', linestyle='--', color='blue')
plt.show()
